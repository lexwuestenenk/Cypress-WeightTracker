...
describe('Create a new account', function () {
    ...
    it('Edit existing workout', function () {
        ...
        workout.getWorkout(name).within(() => {
            // Click workout 
            workout.getWorkoutEdit().click()
        })

        workout.getWorkoutEditForm(name).within(() => {
            // Enter new name
            forms.getFormName()
                .click()
                .clear()
        ...
        // Check if status pop-up has the right status
        generic.getStatus().should('contain', 'Workout has been updated!')
    })

    it('Verify workout was updated', function () {
        // Click register button
        landingPage.getLogin().click()
